            ...
            parallel {
                stage('opg-base-1604') {
                    steps {
                        echo "${env.STAGE_NAME}"
                        script {
                            docker.build("${env.DOCKER_REGISTRY}/temp-opg-base-1604:${env.NEWTAG}", "opg-base-1604")
                        }
                    }
                }
                stage('opg-golang-alpine') {
                    steps {
                        script {
                            docker.build("${env.DOCKER_REGISTRY}/temp-opg-golang-alpine:${env.NEWTAG}", "opg-golang-alpine")
                        }
                    }
                }
                stage('opg-elasticsearch-shared-data-1604') {
                    steps {
                        script {
                            docker.build("${env.DOCKER_REGISTRY}/opg-elasticsearch-shared-data-1604:${env.NEWTAG}", "opg-elasticsearch-shared-data-1604")
                        }
                    }
                }                
            }
