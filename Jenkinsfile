        ...
        stage('base images') {
            parallel {
                stage('opg-base-1604') {
                    steps {
                        script {
                            docker.build("temp-opg-base-1604:${env.NEWTAG}", "opg-base-1604")
                        }
                    }
                }
                stage('opg-golang-alpine') {
                    steps {
                        script {
                            docker.build("temp-opg-golang-alpine:${env.NEWTAG}", "opg-golang-alpine")
                        }
                    }
                }
                stage('opg-elasticsearch-shared-data-1604') {
                    steps {
                        script {
                            docker.build("temp-opg-base-1604:${env.NEWTAG}", "opg-elasticsearch-shared-data-1604")
                        }
                    }
                }                
            }
